...
import type {
  AiSummary, Channel, ChannelState, Message, Reaction,
  ScheduledMessage, SelfUser, SmartReply, TranslationView, User, UserStatus,
} from './types.js';
...
export type ClientEvent =
  ...
  | { t: 'presence:set'; status: UserStatus; statusEmoji?: string | null; statusText?: string | null }
...
export type ServerEvent =
  | { t: 'ready'; self: SelfUser; users: User[]; channels: Channel[]; states: ChannelState[]; scheduled: ScheduledMessage[]; serverTime: number; ai: AiCapabilities }
  | { t: 'pong'; ts: number }
  ...
  | { t: 'presence'; userId: string; status: UserStatus; statusEmoji: string | null; statusText: string | null; lastSeenAt: number | null }
