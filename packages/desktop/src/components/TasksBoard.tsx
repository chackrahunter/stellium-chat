...
export function TaskExtract({ onClose }: { onClose: () => void }) {
  const t = useT();
  const users = useStore((s) => s.users);
  const laeuft = useStore((s) => s.extractingTasks);
  const gefunden = useStore((s) => s.extractedTasks);
  const activeChannelId = useStore((s) => s.activeChannelId);
  const { extractTasks, createTask, clearExtractedTasks } = useStore.getState();
  const [gewaehlt, setGewaehlt] = useState<Set<number>>(new Set());

  useEffect(() => {
    if (activeChannelId && gefunden === null && !laeuft) extractTasks(activeChannelId);
    // Beim Schließen den Vorschlag verwerfen, sonst hängt er beim nächsten Öffnen fest.
    return () => clearExtractedTasks();
  }, [activeChannelId]);

  // Standardmäßig ist alles angehakt — meist will man alles übernehmen.
  useEffect(() => {
    if (gefunden) setGewaehlt(new Set(gefunden.map((_, i) => i)));
  }, [gefunden]);

  const uebernehmen = () => {
    (gefunden ?? []).forEach((v, i) => {
      if (gewaehlt.has(i)) createTask({ title: v.title, assigneeId: v.assigneeId, dueAt: v.dueAt, channelId: activeChannelId });
    });
    onClose();
  };

  return (
    <Shell title={t('ai.extractTitle')} icon={<Sparkles size={18} />} onClose={onClose} width={560}>
      <p className="field__hint" style={{ marginBottom: 12 }}>{t('ai.extractHint')}</p>

      {laeuft && (
        <div className="empty-state">
          <Loader2 size={26} className="spin" />
          <p>{t('ai.extractRunning')}</p>
        </div>
      )}

      {!laeuft && gefunden && !gefunden.length && (
        <div className="empty-state"><p>{t('ai.extractEmpty')}</p></div>
      )}

      {!laeuft && !!gefunden?.length && (
        <>
          <div className="extract-list">
            {gefunden.map((v, i) => (
              <label key={i} className={clsx('extract-row', gewaehlt.has(i) && 'extract-row--on')}>
                <input
                  type="checkbox"
                  checked={gewaehlt.has(i)}
                  onChange={(e) => setGewaehlt((alt) => {
                    const neu = new Set(alt);
                    if (e.target.checked) neu.add(i); else neu.delete(i);
                    return neu;
                  })}
                />
                <span className="extract-row__title">{v.title}</span>
                {v.assigneeId && users[v.assigneeId] && (
                  <span className="extract-row__who">
                    <Avatar user={users[v.assigneeId]} size={16} /> {users[v.assigneeId].displayName}
                  </span>
                )}
                {v.dueAt && <span className="extract-row__due"><CalendarDays size={11} /> {relativeTime(v.dueAt)}</span>}
              </label>
            ))}
          </div>

          <div className="panel__foot">
            <button className="btn btn--primary" disabled={!gewaehlt.size} onClick={uebernehmen}>
              <Check size={14} /> {t('ai.extractAdd')} ({gewaehlt.size})
            </button>
          </div>
        </>
      )}
