 ...
 *   node scripts/e2e-admin.mjs <einmal-passwort>
 */
...
await pruefe('Anmeldung mit Einmal-Passwort', async () => {
  ...
  await seite.fill('input[autocomplete="username"]', 'admin');
  ...
  // Anmeldung und Einrichtung teilen sich dieselbe Klasse.
  await seite.locator('.auth__title', { hasText: 'Willkommen' }).waitFor({ timeout: 20000 });
...
await pruefe('Eigene Zugangsdaten setzen', async () => {
  ...
  await felder.nth(1).fill('don');
  await felder.nth(2).fill('don@stellium.de');
...
await pruefe('Verwaltung öffnen', async () => {
  await seite.locator('.rail button[title="Team verwalten"]').click();
...
await pruefe('Konto anlegen und Einmal-Passwort erhalten', async () => {
  ...
  await felder.nth(0).fill('Sarah Lindqvist');
  ...
  const handle = (await seite.locator('.zugang__wert').first().innerText()).trim();
  ...
  return `@${handle} — ${einmalNeu}`;
});

await pruefe('Rechte einzeln entziehen', async () => {
  const eintrag = seite.locator('.admin__liste .result', { hasText: 'Sarah' }).first();
...
await pruefe('Konto sperren', async () => {
  ...
  await seite.waitForTimeout(900);
  const zeile = seite.locator('.admin__liste .result', { hasText: 'Sarah' }).first();
...
await pruefe('Gesperrtes Konto wird abgewiesen', async () => {
  const r = await seite.evaluate(async (pw) => {
    const res = await fetch('http://localhost:8787/api/auth/login', {
      ...
      body: JSON.stringify({ login: 'sarah.lindqvist', password: pw }),
    });
